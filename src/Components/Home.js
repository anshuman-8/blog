  ...
  return (
    ...
    <div className=" mx-5 md:mx-20">
      ...
      <div className="grid lg:grid-cols-3 md:grid-cols-2 lg:mx-12 justify-center">
        {data.map((item) => 
          (
            <div key={item.id} title={item.title} className="flex flex-col justify-start cursor-pointer  mx-5 md:mx-10 max-w-lg border border-slate-400 rounded-xl dark:bg-slate-200/30 bg-teal-500/5 hover:scale-105 hover:shadow-xl duration-200 my-5 min-w-[250px]" onClick={()=>navigate(`/blog/${item.id}`)} >
                <img
